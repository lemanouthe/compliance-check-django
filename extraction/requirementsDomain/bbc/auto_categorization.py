...
base_dir = os.path.join(BASE_DIR, 'extraction', 'requirementsDomain', 'bbc')
# (
...
def create_data_set():
    with open(os.path.join(base_dir, "data_domain.txt"), "w", encoding="utf-8") as outfile:
        for label in LABELS:
...
def setup_docs():
    docs = []  # (label, text)
    with open(os.path.join(base_dir, "data_domain.txt"), "r", encoding="utf-8") as datafile:
        for row in datafile:
            parts = row.split("\t")
