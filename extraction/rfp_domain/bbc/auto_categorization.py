...
base_dir = os.path.join(BASE_DIR, 'extraction', 'rfp_domain', 'bbc')
...
def create_data_set():
    with open(os.path.join(base_dir, "data_rfp_domain.txt"), "w", encoding="utf-8") as outfile:
        for label in LABELS:
...
def setup_docs():
    docs = []  # (label, text)
    with open(os.path.join(base_dir, "data_rfp_domain.txt"), "r", encoding="utf-8") as datafile:
        for row in datafile:
            parts = row.split("\t")
...
def train_classifier(docs):
    ...
    clf_filename = os.path.join(base_dir, 'naive_bayes_classifier.pkl')
    pickle.dump(naive_bayes_classifier, open(clf_filename, "wb"))

    # also store the vectorizerso we can transform new data
    vec_filename = os.path.join(base_dir, 'count_vectorizer.pkl')
    pickle.dump(vectorizer, open(vec_filename, "wb"))


def classify(text):
    # load classifier
    clf_filename = os.path.join(base_dir, 'naive_bayes_classifier.pkl') #"/Users/mamoutou.doumbia/Desktop/ComplianceCheck/research/rfp_domain/bbc/naive_bayes_classifier.pkl"
    nb_clf = pickle.load(open(clf_filename, "rb"))

    # vectorize the new text
    vec_filename = os.path.join(base_dir, 'count_vectorizer.pkl') #"/Users/mamoutou.doumbia/Desktop/ComplianceCheck/research/rfp_domain/bbc/count_vectorizer.pkl"
